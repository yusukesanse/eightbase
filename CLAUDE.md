...
- **ダーツ / ビリヤード / ポーカー**: **開催日の開始時刻(JST)が締切**（`src/lib/entryDeadline.ts`）。
  ...
  - 開始時刻は日程doc `{game}Schedule.startTime`。日程の登録時（単日・一括登録とも）に毎回入力する
    （シーズン共通の既定時刻の設定は廃止済み）。一括登録は明日以降の登録済みの日の時刻だけを入力した時刻で上書きする
    （今日・過去の登録済みの日の時刻と麻雀の参加費は変わらない）。イレギュラー対応（個別の日だけ時刻を変える）は引き続き
    日程カレンダーで日付ごとに上書きできる。
...
- 卓ごとに **第n半荘（省略可＝続きの番号を自動）と A〜D 卓ラベル**を指定できる（2026-09-07）。同じ半荘に A/B 卓が立つ日を後入力するため。
  省略した卓だけ続き番号を振る（指定した卓は数えない）。ラベル未指定なら `tableLabel` を書かない（当日進行で生成した卓と同じ形）。
...
- ⚠️ **合計が100,000点でなくても管理者は確定できる**（2026-09-07・`{ action: "confirm", force: true }`）。
  紙の記録が欠けている・書き間違いで合計が合わない卓を順位表に載せるための例外。順位は持ち点から振り直す。
  **4名全員の持ち点が入っていることは必須**（未入力は force でも確定しない）。`reviewReason` に合計点を残し、
  監査ログ `table.adminForceConfirmed` に実行者と合計点を必ず書く。UI は「合計が N 点です。管理者権限で確定しますか？」と確認してから送る。
  回帰テスト: `__tests__/unit/api/adminMahjongTableConfirm.test.ts`。
...
- **施設予約は「施設を選択」のブロックだけ以前の UI のまま**、それ以外（見出し RESERVE・カレンダー・時間選択・同伴者・確認・完了・決済・マイ予約・DatePicker／TimePicker）は新デザイン（ユーザー指示・2026-09-07。一度「カレンダーだけ」に戻したが「古臭い」とのことで再度こちらに）。
  カレンダーの**空き状況の点（緑／金）と凡例は出さない**（ユーザー指示）。空きの判定ロジックは残っているが表示は数字だけ。
- リーグのピラミッドは `public/league-pyramid.jpg`（クリスタル画像・M1 マゼンタ／M2 ブルー／M3 ゴールド）を黒のヒーローカードに置く（`LeaguePyramid.tsx`）。**段ラベル（左固定・ゴールド箔風の Noto Serif JP）と「あなた」フラッグ付きで浮遊するアバターは旧 3D 版と同じ表現**（ユーザー指示）。**画像自体は動かさない（ユーザー指示）**。アバターはピラミッドの上（所属する段の面の中央）に重ね、ゆっくり浮遊するだけ（`eb-pyramid-float`・reduced-motion で停止）。位置は `LABEL_TOP` / `AVATAR_TOP` / `PYRAMID_OFFSET_X`。ヒーローは `src/components/LeaguePyramidHero.tsx`（**4種目共通**。麻雀 `LeaguePyramid.tsx` と `games/GameLeagueBoard.tsx` の両方がこれを使う。種目差は tierKeys だけ）。`LeaguePyramid3D.tsx` は未使用で残置。
