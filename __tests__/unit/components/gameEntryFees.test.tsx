...
test("麻雀の単日・一括入力は無効料金で保存不可、日別PATCH可能", async () => {
  ...
  await waitFor(() => expect(fetchMock).toHaveBeenCalledWith("/api/admin/games/schedule", expect.objectContaining({ method: "POST", body: JSON.stringify({ gameCategory: "mahjong", seasonId: "S1", date: "2026-10-02", entryFee: 5000 }) })));
