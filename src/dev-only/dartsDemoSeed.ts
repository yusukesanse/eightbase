...
export async function seedDemoDartsParticipants(seasonId: string): Promise<Record<string, number>> {
  ...
  await db.collection("dartsEntries").doc(`${seasonId}_${today}_${UNPAID.lineUserId}`).set({
    seasonId, eventDate: today, lineUserId: UNPAID.lineUserId, displayName: UNPAID.displayName, pictureUrl: "",
    enteredAt: nowIso, status: "reserved", paymentStatus: "pending", paymentAmount: DARTS_ENTRY_FEE, ...DUMMY_FLAG,
  });
  entryCount++;
