  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-[12px] text-[#231714]/85 leading-relaxed px-0.5">
        隔週木曜が開催日です（18:00〜20:00）。カレンダーの開催日から参加日を選んでください（参加は1か月に1回）。
        {paymentRequired &&
          ...
          if (cancelledDates.has(selectedDate)) {
            return (
              <div
                ...
                style={{ boxShadow: "inset 0 0 0 1.5px #f0c9b0" }}
              >
          ...
          return (
            <>
              <div
                ...
                style={{ boxShadow: `inset 0 0 0 1.5px ${entered ? DARTS_ACCENT : "#eceff1"}` }}
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[14.5px] font-extrabold text-[#231714] truncate">リーグ戦（木曜 18:00〜）</div>
                  ...
                  {needsPay && payStatus === "paid" ? (
                    <div className="shrink-0 flex flex-col items-end gap-1">
                      <span
                        className="inline-flex items-center gap-1 rounded-full text-[12.5px] font-extrabold px-3 py-2 whitespace-nowrap"
                        style={{ background: "#eef4dd", color: "#6f9023" }}
                      >
                        <CheckIcon color="#6f9023" size={13} />
                        参加確定
                      </span>
                      {!isPast && (
                        <button
                          onClick={() => setCancelDate(selectedDate)}
                          className="text-[10.5px] font-bold text-[#231714]/80 underline underline-offset-2 whitespace-nowrap"
                        >
                          支払いをキャンセル
                        </button>
                      )}
                    ...
                    <button
                      onClick={() => toggle(selectedDate, true)}
                      className="shrink-0 text-[11px] font-bold text-[#231714]/80 underline underline-offset-2 whitespace-nowrap"
                    >
                      参加をやめる
                    </button>
                  ) : dateFull ? (
                    <span className="shrink-0 inline-flex items-center rounded-full text-[12.5px] font-extrabold px-3 py-2 bg-[#231714]/5 text-[#231714]/80">
                      満員
                    </span>
                  ) : !isPast ? (
                    <button
                      ...
                      style={{
                        background: DARTS_ACCENT,
                        color: "#fff",
                        boxShadow: `0 2px 8px color-mix(in srgb, ${DARTS_ACCENT} 40%, transparent)`,
                      }}
                    >
                ...
                {unpaidNotice && (
                  <div className="flex items-stretch gap-2">
                    <button
                      ...
                      style={{ background: DARTS_CONFIRM, boxShadow: `0 2px 8px color-mix(in srgb, ${DARTS_CONFIRM} 40%, transparent)` }}
                    >
                      {busy === selectedDate ? "..." : `支払いする ¥${DARTS_ENTRY_FEE.toLocaleString()}`}
                    </button>
                    <button
                      onClick={() => toggle(selectedDate, true)}
                      disabled={busy === selectedDate}
                ...
                return (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <span
                      ...
                      style={paid ? { background: "#eef4dd", color: "#6f9023" } : { background: "#fdf4e3", color: "#b48f13" }}
                    >
                      {paid ? "支払い済み" : "参加済み（未払い）"}
                    </span>
                  </div>
