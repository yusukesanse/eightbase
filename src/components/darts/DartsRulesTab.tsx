...
/**
 * ダーツ ルール/約款タブ（閲覧のみ・同意フローは持たない）。麻雀 MahjongRulesTab の読み替え。
 * 内容は管理画面のシーズン設定で Markdown 登録（種目ごと × シーズンごと）。未登録は切替に出さない。
 */

type Doc = "rules" | "terms";
const DOC_LABEL: Record<Doc, string> = { rules: "ルール", terms: "約款" };

export function DartsRulesTab() {
  ...
  useEffect(() => {
    let alive = true;
    fetch("/api/games/rules?gameCategory=darts", { credentials: "include", cache: "no-store" })
      .then((r) => r.json())
      .then((d) => {
        if (!alive) return;
  ...
  if (error) {
    return <div className="py-10 text-center text-sm text-[#d8533a]">{error}</div>;
  }
        ...
        <div className="flex gap-1 bg-[#231714]/5 rounded-xl p-1">
          {available.map((d) => (
            <button
              ...
              style={doc === d ? { color: DARTS_ACCENT } : undefined}
            >
              {DOC_LABEL[d]}
            </button>
          ))}
