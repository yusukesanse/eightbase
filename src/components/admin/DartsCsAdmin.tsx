...
/**
 * ダーツCS 管理（作成・一覧）。§5＝GMなし完全自動進行なので、管理は
 * 「名称＋エントリー締切日（=自動ブラケット生成の起点）」を作るだけ。進行・確定は不要。
 * 誰でも参加可なので entrants は空で開始し、利用者が「CS」タブから自己エントリーする。
 */
export default function DartsCsAdmin() {
  const { seasonId } = useParams<{ seasonId: string }>();
  const [events, setEvents] = useState<DartsCsEvent[]>([]);
  ...
  const load = useCallback(() => {
    setLoading(true);
    fetch(`/api/admin/darts/cs?seasonId=${seasonId}`, { credentials: "same-origin" })
      .then((r) => r.json())
  ...
  async function create() {
    ...
    try {
      const res = await fetch("/api/admin/darts/cs", {
        method: "POST",
