...
/**
 * ポーカーCS 管理（作成・一覧）。§5＝GMなし完全自動進行なので、管理は
 * 「名称＋エントリー締切日（=自動ブラケット生成の起点）」を作るだけ。進行・確定は不要。
 * 誰でも参加可なので entrants は空で開始し、利用者が「CS」タブから自己エントリーする。
 */
export default function PokerCsAdmin() {
  const { seasonId } = useParams<{ seasonId: string }>();
  const [events, setEvents] = useState<PokerCsEvent[]>([]);
  ...
  const load = useCallback(() => {
    setLoading(true);
    fetch(`/api/admin/poker/cs?seasonId=${seasonId}`, { credentials: "same-origin" })
      .then((r) => r.json())
  ...
  async function create() {
    ...
    try {
      const res = await fetch("/api/admin/poker/cs", {
        method: "POST",
  ...
  return (
    <div className="p-4 flex flex-col gap-4 max-w-2xl">
      <p className="text-sm text-[#231714]/80 leading-relaxed">
        チャンピオンシップは<b>GMなしの完全自動進行</b>です。名称と<b>エントリー締切日</b>を設定して作成すると、
        締切日の到来で自動的にトーナメント表が生成されます（リーグ上位4名は予選免除シード）。
        参加は利用者が「CS」タブから自己エントリーします（誰でも参加可・無料）。
      </p>

      {msg && <div className={`rounded-xl px-4 py-2.5 text-sm font-bold ${msg.ok ? "bg-[#eef4dd] text-[#5f7d1e]" : "bg-[#fdece8] text-[#d8533a]"}`}>{msg.text}</div>}
          ...
          <ul className="flex flex-col divide-y divide-gray-100">
            {events.map((e) => (
              <li key={e.csEventId} className="flex items-center justify-between py-2.5">
                ...
                <span className="text-xs font-bold px-2 py-0.5 rounded-full" style={{ background: "#eef4f5", color: "#3c4f54" }}>{STATUS_LABEL[e.status] ?? e.status}</span>
              </li>
            ))}
          </ul>
        )}
