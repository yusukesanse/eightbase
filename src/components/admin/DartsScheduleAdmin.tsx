...
/**
 * ダーツ 開催日（日程）管理。麻雀の「毎週土曜＋休催トグル」と違い、
 * ダーツは開催日を明示登録する（隔週木曜が既定）。`/api/admin/darts/schedule` を叩く。
 * ※ POST はアクティブなダーツシーズンに登録する（API 仕様）。編集はアクティブシーズンで行うこと。
 */
export default function DartsScheduleAdmin() {
  const { seasonId } = useParams<{ seasonId: string }>();
  const [schedule, setSchedule] = useState<DartsScheduleEntry[]>([]);
  ...
  const load = useCallback(() => {
    setLoading(true);
    fetch(`/api/admin/darts/schedule?seasonId=${seasonId}`, { credentials: "same-origin" })
      .then((r) => r.json())
  ...
  async function addSingle() {
    ...
    try {
      const res = await fetch("/api/admin/darts/schedule", {
        method: "POST",
  ...
  async function addBulk() {
    ...
    try {
      ...
      if (!res.ok) setMsg({ ok: false, text: data.error ?? "一括登録に失敗しました" });
        ...
        setMsg({ ok: true, text: `隔週木曜を${data.added}件登録しました` });
        load();
      }
    } finally {
      setBusy(false);
  ...
  async function remove(date: string) {
    ...
    setBusy(true); setMsg(null);
    try {
      const res = await fetch(`/api/admin/darts/schedule?seasonId=${seasonId}&date=${date}`, {
        method: "DELETE",
        credentials: "same-origin",
      });
      const data = await res.json().catch(() => ({}));
  ...
  return (
    <div className="p-4 flex flex-col gap-4 max-w-2xl">
      <p className="text-sm text-[#231714]/80 leading-relaxed">
        ダーツの開催日を登録します（既定は隔週木曜 {DARTS_DEFAULT_START_TIME}〜{DARTS_DEFAULT_END_TIME}）。
        ここに登録された日だけが利用者アプリで参加可能になります。祝日等はスキップして個別登録してください。
      </p>

      {msg && (
        <div className={`rounded-xl px-4 py-2.5 text-sm font-bold ${msg.ok ? "bg-[#eef4dd] text-[#5f7d1e]" : "bg-[#fdece8] text-[#d8533a]"}`}>
          {msg.text}
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex flex-col gap-3">
        <div className="text-sm font-bold text-[#231714]">隔週木曜を一括登録</div>
        <div className="flex flex-wrap items-end gap-3">
          <div className="flex flex-col gap-1">
            <label className="text-xs font-bold text-[#231714]/70">起点日（以降の最初の木曜から隔週）</label>
