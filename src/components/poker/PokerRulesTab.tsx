...
/**
 * ポーカー ルール/約款タブ（閲覧のみ）。ダーツ/ビリヤードの RulesTab の読み替え。
...
export function PokerRulesTab() {
  ...
  useEffect(() => {
    let alive = true;
    fetch("/api/games/rules?gameCategory=poker", { credentials: "include", cache: "no-store" })
      .then((r) => r.json())
      .then((d) => {
        if (!alive) return;
        ...
        <div className="flex gap-1 bg-[#231714]/5 rounded-xl p-1">
          {available.map((d) => (
            <button
              ...
              style={doc === d ? { color: POKER_ACCENT } : undefined}
            >
              {DOC_LABEL[d]}
            </button>
          ))}
