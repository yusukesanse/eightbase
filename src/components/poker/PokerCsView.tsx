 ...
 * 卓分け→勝ち上がり→決勝卓。申告は**終了時チップ**のみ（順位は chips 降順で派生）。
 * 上位4名は予選免除シード（S）。各卓1位が勝ち上がり、1位同点は追加ハンド。決勝卓は 1位=金/2位=銀/3位=銅。
 * ※ CSは**ディーラーを固定しない**（卓の中で交代しながら回す）。卓の全員がチップを申告する。
 */

interface PubPlayer { displayName: string; pictureUrl?: string; chips: number | null; rank: number | null; seed: boolean; isMe: boolean }
...
export function PokerCsView() {
  ...
  const load = useCallback(() => {
    return fetch("/api/poker/cs", { credentials: "include" })
      .then((r) => r.json())
  ...
  const report = useCallback(
    async (csEventId: string, matchId: string, chips: number) => {
      setBusy(true); setInputError(null);
      try {
        const res = await fetch("/api/poker/cs/report", {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({ csEventId, matchId, chips }),
        });
  ...
  const toggleEntry = useCallback(
    async (join: boolean) => {
      ...
      try {
        const res = await fetch("/api/poker/cs/entry", { method: join ? "POST" : "DELETE", credentials: "include" });
        const data = await res.json().catch(() => ({}));
    ...
    <div className="flex flex-col gap-4">
      ...
      {inputMatch && (
        <CsInputSheet
          ...
          onReport={(chips) => report(event.csEventId, inputMatch.matchId, chips)}
        />
      )}
    </div>
  );
...
/** 結果申告シート（終了時チップ or 追加ハンド）。順位はサーバーが chips から派生。 */
function CsInputSheet({ match, busy, error, onClose, onReport }: { match: PubMatch; busy: boolean; error: string | null; onClose: () => void; onReport: (chips: number) => void }) {
  const tiebreak = match.status === "tiebreak";
