...
import { dateParts, formatJpDate, todayJst } from "@/components/billiards/billiardsShared";
    ...
    <div className="flex flex-col gap-4">
      ...
      {selectedDate && !cancelledDates.has(selectedDate) && selectedDate < today && dayStandings && (
        dayStandings.hasResults ? (
          <BilliardsDayStandings eventDate={selectedDate} standings={dayStandings.standings} />
        ) : (
          <GlassCard>
...
function SelectedDateCard({
  ...
  entered: boolean;
  paymentRequired: boolean;
  payStatus: BilliardsPaymentStatus | null;
  cancelled: boolean;
  isPast: boolean;
  startTime?: string;
  endTime?: string;
      ...
      <GlassCard tone="coral">
        <div className="flex flex-col gap-3">
          ...
          <p className="text-[15px] leading-relaxed text-[color:var(--eb-ink)]">
            この開催日は中止になりました。{entered && "お支払い済みの参加費は返金対応します。"}
          </p>
        </div>
      </GlassCard>
    );
    ...
    return (
      <GlassCard tone="gold">
        <div className="flex flex-col gap-3">
          ...
          <p className="text-[15px] leading-relaxed text-[color:var(--eb-ink)]">
            参加枠を確保しました。当日プレイするには参加費（¥{BILLIARDS_ENTRY_FEE.toLocaleString()}）のお支払いが必要です。
          </p>
          ...
          <Button variant="pay" loading={busy} onClick={onPay}>
            支払いする ¥{BILLIARDS_ENTRY_FEE.toLocaleString()}
          </Button>
          <Button variant="ghost" loading={busy} onClick={onLeave}>
            参加をやめる
          </Button>
  ...
  const reason = isPast ? "この開催日は終了しました" : dateFull ? "満員です" : "この日に参加できます";
