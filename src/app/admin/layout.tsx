...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#f0f4f5] flex">
      ...
      >
        <div className="flex flex-col items-center h-full w-[72px] py-4 bg-white/80 backdrop-blur-xl border-r border-[#A5C1C8]/25">
          {/* ロゴ */}
                ...
                <div key={link.href} className="relative">
                  <Link
                    ...
                    className={`
                      ...
                      ${isActive
                        ? "bg-[#A5C1C8] text-white shadow-md shadow-[#A5C1C8]/40"
                        : "text-[#231714]/40 hover:text-[#231714] hover:bg-[#A5C1C8]/20"
                      }
                    `}
                  >
                    {link.icon}
      ...
      <main className="flex-1 overflow-auto min-h-screen">
        {/* モバイルヘッダー */}
        <div className="md:hidden sticky top-0 z-30 px-4 py-3 bg-white/80 backdrop-blur-xl border-b border-[#A5C1C8]/25">
          <div className="flex items-center justify-between">
            <button
              onClick={() => setSidebarOpen(true)}
              className="p-2 rounded-xl hover:bg-[#A5C1C8]/20 transition-colors text-[#231714]"
            >
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
