    ...
    <div className="p-8">
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#231714]/80 mt-1">ランキング集計期間の作成・管理</p>
        </div>
            ...
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-[#231714]/85">
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 w-full max-w-sm">
            ...
            <p className="text-sm text-[#231714]/80 mb-5">
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg my-8">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <p className="text-[10px] text-[#231714]/80 mt-1">
              ...
              <div>
                ...
                <p className="text-xs text-gray-700 mt-1">
                  保存すると<b>今後の開催日にも反映</b>されます（「日程」カレンダーで個別に変更した日は保護され、そのまま）。
                  <b>イレギュラーな日だけ「日程」カレンダーで個別に変更</b>できます。未入力なら種目の既定値を使います。
                </p>
                <p className="text-xs mt-1" style={{ color: form.gameCategory === "mahjong" ? "#6b7280" : "#a1702c" }}>
                  {form.gameCategory === "mahjong"
                    ? "麻雀は受付締切をGMの「ゲーム開始」で行うため、この時刻は表示・目安です（締切には影響しません）。"
                    : "この種目はGMを事前に決めないため、開始時刻が**参加受付の締切**になります。利用者には「開始時刻までに支払う」よう案内されます。"}
                </p>
              </div>
                ...
                <div>
                      ...
                      {form.gameMasterIds.map((id) => {
                        ...
                        return (
                          <span key={id} className="inline-flex items-center gap-1 bg-[#231714]/5 rounded-full pl-2.5 pr-1.5 py-1 text-xs text-[#231714]">
                            {c?.displayName ?? id}
                            <button type="button" onClick={() => toggleGm(id)} className="text-[#231714]/80 hover:text-red-500 leading-none">×</button>
                          </span>
                        );
                      })}
                    </div>
                  ...
                  <div className="max-h-40 overflow-y-auto border border-gray-100 rounded-lg divide-y divide-gray-50">
                    ...
                    {gmCandidates.length === 0 && (
                      <div className="px-3 py-3 text-xs text-gray-700">LINE連携済みの利用者がいません。</div>
                    )}
                  </div>
                </div>
              )}
