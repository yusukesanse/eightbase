...
import { resolveDartsSeasonRank, DARTS_CS_NON_LEAGUE_RANK } from "@/lib/dartsCsServer";
import { DARTS_CS_SEED_COUNT } from "@/lib/dartsCs";
import type { DartsCsEvent, DartsCsEntrant } from "@/types/darts";

export const dynamic = "force-dynamic";

/** アクティブシーズンの最新CS doc の ref を返す（無ければ null 情報つき）。 */
async function resolveCurrentCs(
  db: FirebaseFirestore.Firestore,
  seasonId: string
): Promise<{ ref: FirebaseFirestore.DocumentReference } | null> {
  const snap = await db.collection("dartsCsEvents").where("seasonId", "==", seasonId).get();
...
/** POST /api/darts/cs/entry — CSに自己エントリー（setup中のみ・冪等）。 */
export async function POST(req: NextRequest) {
  try {
    ...
    // 非競合の事前読み取り（seed 順位・プロフィール）。
    const rank = await resolveDartsSeasonRank(season.seasonId, userId);
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      const ev = doc.data() as DartsCsEvent;
      ...
      const entrant: DartsCsEntrant = {
        lineUserId: userId,
        displayName: u.displayName || "ユーザー",
        pictureUrl: u.pictureUrl || "",
        rank,
        seed: rank <= DARTS_CS_SEED_COUNT && rank !== DARTS_CS_NON_LEAGUE_RANK,
      };
    ...
    console.error("[darts/cs/entry] POST error:", error);
    return NextResponse.json({ error: "エントリーに失敗しました" }, { status: 500 });
  }
}

/** DELETE /api/darts/cs/entry — エントリー取消（setup中のみ・冪等）。 */
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.error("[darts/cs/entry] DELETE error:", error);
