...
/**
 * GET /api/admin/billiards/refunds
 * 返金対応が必要/処理済みのビリヤード参加費エントリー一覧（全シーズン横断）。
...
export async function GET(req: NextRequest) {
  ...
  const [byStatus, byPayment] = await Promise.all([
    db.collection("billiardsEntries").where("status", "in", ["cancelRequested", "refunded", "cancelRejected"]).get(),
    db.collection("billiardsEntries").where("paymentStatus", "==", "cancelRequested").get(),
  ]);

  const map = new Map<string, BilliardsEntry & { entryId: string }>();
  for (const d of [...byStatus.docs, ...byPayment.docs]) {
    map.set(d.id, { ...(d.data() as BilliardsEntry), entryId: d.id });
  }
