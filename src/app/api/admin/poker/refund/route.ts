...
import { canTransition, deriveStatus, type PokerEntryStatus } from "@/lib/pokerEntryStatus";
...
/**
 * POST /api/admin/poker/refund  Body: { entryId, action: "refund" | "reject" }
...
export async function POST(req: NextRequest) {
  ...
  const from = deriveStatus(snap.data() as PokerEntry);
  const to: PokerEntryStatus = action === "refund" ? "refunded" : "cancelRejected";
