...
import { removeBilliardsParticipant } from "@/lib/billiardsDay";
...
/**
 * DELETE /api/billiards/day/participant  Body: { eventDate, targetUserId }
 * 当日GM専用: 参加者を外す（参加剥奪）。来ない人・参加費を払わない人で進行が止まるのを防ぐ。
 * 確定済み（記録済みの試合）がある場合は成績が壊れるため拒否する。
 */
export async function DELETE(req: NextRequest) {
  ...
  try {
    const result = await removeBilliardsParticipant(season.seasonId, eventDate, targetUserId);
    ...
    console.error("[billiards/day/participant] DELETE error:", error);
