  ...
  try {
    ...
    try {
      await db.runTransaction(async (tx) => {
        ...
        const heldSeat = entrySnap.exists && deriveStatus(entrySnap.data() as BilliardsEntry) !== "refunded";
        if (!heldSeat) {
          ...
          const active = dateSnap.docs.filter((d) => d.id !== entryId && deriveStatus(d.data() as BilliardsEntry) !== "refunded").length;
          if (active >= BILLIARDS_MAX_ENTRIES_PER_DATE) throw new Error("FULL");
        }
