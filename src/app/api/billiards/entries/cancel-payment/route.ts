...
/**
 * POST /api/billiards/entries/cancel-payment  Body: { eventDate }
 * 支払い済み参加費のキャンセル依頼。**自動返金はしない**。cancelRequested にし管理者へ手動返金通知。
 * キャンセル期限は開催日の7日前まで（麻雀/ダーツと共通）。
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const entry = { ...(snap.data() as BilliardsEntry), entryId };
    ...
    console.error("[billiards/entries/cancel-payment] POST error:", message, err);
