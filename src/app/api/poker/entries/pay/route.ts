...
import { getPokerDayState } from "@/lib/pokerDay";
 ...
 * 戻り先は /games?pokerpay=エントリーID → /api/poker/entries/complete が確定する。
...
export async function POST(req: NextRequest) {
  try {
    ...
    if ((await getPokerDayState(season.seasonId, eventDate))?.finishedAt) {
    ...
    const completePath = gamePaymentReturnPath("poker", entryId);
