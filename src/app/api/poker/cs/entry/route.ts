...
import { resolvePokerSeasonRank, POKER_CS_NON_LEAGUE_RANK } from "@/lib/pokerCsServer";
import { POKER_CS_SEED_COUNT } from "@/lib/pokerCs";
...
async function resolveCurrentCs(
  ...
  const snap = await db.collection("pokerCsEvents").where("seasonId", "==", seasonId).get();
...
/** POST /api/poker/cs/entry — CSに自己エントリー（setup中のみ・冪等）。 */
export async function POST(req: NextRequest) {
  try {
    ...
    const rank = await resolvePokerSeasonRank(season.seasonId, userId);
    ...
    const result = await db.runTransaction(async (tx) => {
      ...
      const ev = doc.data() as PokerCsEvent;
      ...
      const entrant: PokerCsEntrant = {
        lineUserId: userId,
        displayName: u.displayName || "ユーザー",
        pictureUrl: u.pictureUrl || "",
        rank,
        seed: rank <= POKER_CS_SEED_COUNT && rank !== POKER_CS_NON_LEAGUE_RANK,
      };
    ...
    console.error("[poker/cs/entry] POST error:", error);
    return NextResponse.json({ error: "エントリーに失敗しました" }, { status: 500 });
  }
}

/** DELETE /api/poker/cs/entry — エントリー取消（setup中のみ・冪等）。 */
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.error("[poker/cs/entry] DELETE error:", error);
