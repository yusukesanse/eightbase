/**
 * ポーカー エントリー系 API 共通の入力バリデーション・IDヘルパー・開催日ユーティリティ。
 * ★ 有効な開催日は `pokerSchedule` コレクション（管理登録＝第1/第3土曜）が正。
 *   ここには曜日の純関数（第1/第3土曜の生成・土曜判定）だけを置く。
 */
...
export function buildPokerEntryId(seasonId: string, eventDate: string, lineUserId: string): string {
  return `${seasonId}_${eventDate}_${lineUserId}`;
}

export function buildPokerScheduleId(seasonId: string, date: string): string {
...
/**
 * 起点月以降の「第1・第3土曜」を count 個生成（管理画面の一括登録用・昇順）。
 * 起点が YYYY-MM-DD のときは、その日以降の第1/第3土曜だけを含める。
 */
export function generateFirstThirdSaturdays(startDate: string, count: number): string[] {
