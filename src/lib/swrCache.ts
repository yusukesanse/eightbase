...
export const CACHE_TTL: Record<string, number> = {
  ...
  // ⚠️ 施設は「ほぼ静的」ではない。**管理画面でいつでも編集できる**。
  //    10分キャッシュにしていたため、管理画面で営業時間・曜日・料金・同伴者必須などを変えても
  //    利用者アプリが最大10分間**古い設定のまま**になり、「管理画面の設定と利用者アプリが
  //    一致しない」「存在しない枠が選べる／選べるはずの枠が出ない」という事故になった。
  //    ttl:0 = 前回値を即表示しつつ**毎回サーバーへ取り直す**（体感速度は落とさず鮮度を担保）。
  facilities: 0,
