...
2. API Keys からキーを取得し `RESEND_API_KEY` に設定
3. Domains で送信元ドメイン（eightbase.net）を追加し、DNS レコード（DKIM / SPF）を設定して認証
...
```
【管理者】ユーザー招待（名前 + メールアドレス）
    │
    ▼
ワンタイムパスワード発行 → メールで自動送信（Resend）
    │
    ▼
【利用者】LINE ミニアプリ起動 → LIFF 自動ログイン
    │
    ▼
...
| ユーザー管理 | メール招待・有効/無効・パスワード再発行・CSV出力・顧客詳細 |
